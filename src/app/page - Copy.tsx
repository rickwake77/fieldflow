  ...
  return (
    <div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <Card className="p-5">
          <h3 className="text-sm font-bold text-stone-900 mb-4" style={{ fontFamily: "Georgia, serif" }}>Recent Jobs</h3>
          <div className="space-y-1">
            {jobs.slice(0, 6).map((job: any) => (
              <div key={job.id} className="flex justify-between items-center gap-3 py-2.5 border-b border-stone-100 last:border-0">
                <div className="min-w-0 flex-1">
        ...
        <Card className="p-5">
          <h3 className="text-sm font-bold text-stone-900 mb-4" style={{ fontFamily: "Georgia, serif" }}>Team</h3>
          <div className="space-y-1">
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b-2 border-stone-200">
              {["Job", "Customer", "Field", "Type", "Assigned To", "Date", "Qty", "Status"].map(h => (
          ...
          <tbody>
            {filtered.map((job: any) => (
              <tr key={job.id} className="border-b border-stone-100 last:border-0 hover:bg-stone-50 cursor-pointer transition" onClick={() => onSelectJob(job)}>
                ...
                <td className="px-4 py-3 text-sm font-mono">{job.estimatedQuantity ? `${Number(job.estimatedQuantity)} ${job.unitType || ""}` : "—"}</td>
        ...
        {filtered.map((job: any) => (
          <Card key={job.id} className="p-4" onClick={() => onSelectJob(job)}>
            ...
            <div className="text-xs text-stone-500 space-y-0.5">
              <div className="truncate">{job.customer?.name} · {job.field?.fieldName}</div>
              <div>{fmtDate(job.plannedDate)} · {job.estimatedQuantity ? `${Number(job.estimatedQuantity)} ${job.unitType || ""}` : ""}</div>
            </div>
          </Card>
        ))}
        {filtered.length === 0 && <div className="text-center py-12 text-stone-400 text-sm">No jobs found</div>}
      ...
      <Modal isOpen={showCreate} onClose={() => setShowCreate(false)} title="Create New Job">
        ...
        <FormField label="Field" required>
          <select className={inputClass} value={form.fieldId} onChange={e => setForm(f => ({ ...f, fieldId: e.target.value }))} disabled={!form.customerId}>
            <option value="">{form.customerId ? "Select field..." : "Select customer first"}</option>
        ...
        <FormField label="Job Type" required>
          <select className={inputClass} value={form.jobTypeId} onChange={e => handleJobTypeChange(e.target.value)}>
            ...
            {jobTypes.map((jt: any) => <option key={jt.id} value={jt.id}>{jt.name} ({fmtCurrency(Number(jt.defaultRate))}/{jt.billingUnit})</option>)}
          </select>
        </FormField>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <FormField label={`Est. Quantity${form.unitType ? ` (${form.unitType}s)` : ""}`}>
            <input className={inputClass} type="number" step="0.1" placeholder="0" value={form.estimatedQuantity} onChange={e => setForm(f => ({ ...f, estimatedQuantity: e.target.value }))} />
          </FormField>
        </div>
        ...
        <div className="flex gap-2 mt-2">
          ...
          <Btn className="flex-[2]" onClick={handleCreate} disabled={creating || !form.customerId || !form.fieldId || !form.jobTypeId || !form.title}>
    ...
    <div>
      ...
      <Card className="p-5 mb-4">
        ...
        <div className="grid sm:grid-cols-2 gap-x-8 gap-y-2 text-sm">
          ...
          <div><span className="text-stone-500">Field:</span> <span className="font-medium">{job.field?.fieldName} ({Number(job.field?.hectares)} ac)</span></div>
          ...
          <div><span className="text-stone-500">Estimate:</span> <span className="font-medium">{estQty} {job.unitType}s</span></div>
        </div>
      ...
      <Card className="p-5 mb-4">
        ...
        <div className="flex gap-8 mb-3">
          <div>
            ...
            <span className="text-sm text-stone-500">/{estQty} {job.unitType}s</span>
          </div>
          <div>
            ...
            {logs.map((log: any) => (
              <Card key={log.id} className="p-4">
                ...
                <div className="text-sm text-stone-500 mt-1">
                  {Number(log.quantityCompleted)} {job.unitType}s · {Number(log.hoursWorked)} hrs
      ...
      <Modal isOpen={showEditForm} onClose={() => setShowEditForm(false)} title="Edit Job">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <FormField label={`Est. Quantity${job.unitType ? ` (${job.unitType}s)` : ""}`}>
            <input className={inputClass} type="number" step="0.1" value={editForm.estimatedQuantity} onChange={e => setEditForm(f => ({ ...f, estimatedQuantity: e.target.value }))} />
          </FormField>
        </div>
        <FormField label="Description">
                ...
                {completedJobs.map((job: any) => (
                  <label key={job.id} className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition ${selectedJobIds.includes(job.id) ? "border-field-400 bg-field-50" : "border-stone-200 hover:bg-stone-50"}`}>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-stone-500">{job.field?.fieldName} · {job.jobType?.name}</div>
                    </div>
                  </label>
                ))}
              </div>
...
function TeamView() {
  ...
  const [showCreate, setShowCreate] = useState(false);
  const [creating, setCreating] = useState(false);
  const [form, setForm] = useState({ name: "", email: "", phone: "", password: "", role: "contractor" as string });
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (!data.success) {
        ...
        await refresh();
        setShowCreate(false);
        setForm({ name: "", email: "", phone: "", password: "", role: "contractor" });
      }
    } catch {
  ...
  return (
    <div>
            ...
            {admins.map((u: any) => (
              <Card key={u.id} className="p-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-harvest-100 flex items-center justify-center text-harvest-700 font-bold text-sm">
                  ...
                  <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-harvest-50 text-harvest-700">Admin</span>
                </div>
              </Card>
            ))}
          </div>
      ...
      <div>
        ...
        <div className="space-y-2">
          {contractors.map((u: any) => (
            <Card key={u.id} className="p-4">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-field-100 flex items-center justify-center text-field-700 font-bold text-sm">
                  {u.name.split(" ").map((n: string) => n[0]).join("")}
                </div>
                <div className="flex-1">
                  <div className="font-semibold text-sm">{u.name}</div>
                ...
                <div className="text-xs text-stone-400">{u.phone || "—"}</div>
                <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${u.active ? "bg-emerald-50 text-emerald-600" : "bg-stone-100 text-stone-500"}`}>
      ...
      <Modal isOpen={showCreate} onClose={() => setShowCreate(false)} title="Add Team Member">
        ...
        <FormField label="Email" required>
          <input className={inputClass} type="email" placeholder="jack@hendersonag.co.uk" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} />
        </FormField>
        <div className="grid grid-cols-2 gap-3">
          <FormField label="Phone">
        ...
        <div className="flex gap-2 mt-2">
          <Btn variant="ghost" className="flex-1" onClick={() => setShowCreate(false)}>Cancel</Btn>
          <Btn className="flex-[2]" onClick={handleCreate} disabled={creating || !form.name || !form.email || !form.password}>
...
function Sidebar({ currentView, setView, session }: { currentView: ViewId; setView: (v: ViewId) => void; session: any }) {
  ...
  const links: { id: ViewId; label: string; icon: string; adminOnly?: boolean }[] = [
    ...
    { id: "customers", label: "Customers", icon: "M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2M9 11a4 4 0 100-8 4 4 0 000 8M23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75" },
    { id: "invoices", label: "Invoices", icon: "M1 4h22v16H1zM1 10h22", adminOnly: true },
    { id: "job-types", label: "Job Types", icon: "M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" },
    { id: "machines", label: "Machines", icon: "M7 17a3 3 0 100-6 3 3 0 000 6zM19 17a2 2 0 100-4 2 2 0 000 4zM5 17H3V9l4-4h6l3 4h5v8h-2" },
    { id: "team", label: "Team", icon: "M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2M9 11a4 4 0 100-8 4 4 0 000 8M20 8v6M23 11h-6", adminOnly: true },
  ];
    ...
    <aside className="fixed left-0 top-0 bottom-0 w-64 bg-field-900 text-white hidden lg:flex flex-col z-50">
      <div className="px-5 py-6 border-b border-white/10">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <div className="font-bold text-base" style={{ fontFamily: "Georgia, serif" }}>FieldFlow</div>
...
export default function FieldFlowApp() {
  ...
  const loadAll = useCallback(async () => {
    try {
      ...
      setJobs(isAdmin ? j : j.filter((job: any) => job.assignedTo?.id === currentUserId));
      setJobTypes(jt);
      setMachines(m);
      setInvoices(i);
    } catch (err) {
  ...
  const renderContent = () => {
    switch (currentView) {
      ...
      case "invoices": return isAdmin ? <InvoicesView /> : <Dashboard />;
      case "job-types": return <JobTypesView />;
      case "machines": return <MachinesView />;
      case "team": return isAdmin ? <TeamView /> : <Dashboard />;
